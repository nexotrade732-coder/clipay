      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-6 py-5 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl px-4 py-2 outline-none focus:border-blue-500"
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredTransactions.slice(0, 100).map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="px-6 py-4 text-xs font-mono">{tx.user_id?.slice(0, 8)}...</td>
